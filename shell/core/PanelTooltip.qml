...
PopupWindow {
    ...
    anchor.onAnchoring: {
        ...
        const screenX = root.anchorWindow.screen && root.anchorWindow.screen.x !== undefined ? root.anchorWindow.screen.x : 0;
        root.anchorX = point.x - screenX;
        anchor.rect.x = Math.round(Math.max(0, Math.min(root.anchorWindow.width - root.width, root.rightAligned ? root.anchorX - root.width : root.anchorX))) | 0;
    }
